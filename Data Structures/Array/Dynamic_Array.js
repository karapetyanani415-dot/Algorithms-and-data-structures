...
class DArray {
    ...
    resize(newCapacity, fill = 0) {
        ...
        if (!Number.isInteger(newCapacity)) throw new Error("newCapacity must be integer");
    ...
    pop_back() {
        ...
        if (this.empty()) if (this.empty()) return undefined;
    ...
    at(index) {
        ...
        if (index < 0 || index > this.#size - 1) throw new Error("Out of range")
